...
createHITables_fun <- function(tables, filetype = 'csv'){
  ...
  outcome.tables <- tables[tables %in% c('donor', 'isolation', 'gsis', 'peri_gluc', 'peri_leu', 'peri_olp', 'ephys_donor', 'seahorse','grs')]
...
getDonorInfo <- function(record_id, dataType,version="1"){
  ...
  if(file.exists(file.nm)) { # return name; do not recreate file
    ...
    if(dataType == "donor"){
      ...
      if(version=="2"){
        ...
        donor.res$t2d_grs = list(value = round(info$t2d_grs, 2), perc = getPercentile(info$t2d_grs, all.info$t2d_grs))
        
        donor.res$t2d_beta_cell = list(value = round(info$t2d_beta_cell,2), perc = getPercentile(info$t2d_beta_cell, all.info$t2d_beta_cell))
        donor.res$t2d_proins = list(value = round(info$t2d_proins, 2), perc = getPercentile(info$t2d_proins, all.info$t2d_proins))
        donor.res$t2d_obesity = list(value = round(info$t2d_obesity,2), perc = getPercentile(info$t2d_obesity, all.info$t2d_obesity))
        donor.res$t2d_lipodys = list(value = round(info$t2d_lipodys, 2), perc = getPercentile(info$t2d_lipodys, all.info$t2d_lipodys))
        donor.res$t2d_liver_lipid = list(value = round(info$t2d_liver_lipid,2), perc = getPercentile(info$t2d_liver_lipid, all.info$t2d_liver_lipid))
        
        donor.res$t1d_drdq = list(value = round(info$t1d_drdq,2), perc = getPercentile(info$t1d_drdq, all.info$t1d_drdq))
        donor.res$t1d_class1 = list(value = round(info$t1d_class1, 2), perc = getPercentile(info$t1d_class1, all.info$t1d_class1))
        donor.res$t1d_class2 = list(value = round(info$t1d_class2,2), perc = getPercentile(info$t1d_class2, all.info$t1d_class2))
        donor.res$t1d_nonhla = list(value = round(info$t1d_nonhla, 2), perc = getPercentile(info$t1d_nonhla, all.info$t1d_nonhla))
      ...
      parsc = info$t2d_beta_cell+info$t2d_proins+info$t2d_obesity+info$t2d_lipodys+info$t2d_liver_lipid;
      
      donor.res$t2d_beta_cell = list(value = round(info$t2d_beta_cell,2), perc = getPercentile(info$t2d_beta_cell, all.info$t2d_beta_cell),par_risk= round(info$t2d_beta_cell/parsc,2))
      ...
      donor.res$t2d_obesity = list(value = round(info$t2d_obesity,2), perc = getPercentile(info$t2d_obesity, all.info$t2d_obesity),par_risk= round(info$t2d_obesity/parsc,2))
      donor.res$t2d_lipodys = list(value = round(info$t2d_lipodys, 2), perc = getPercentile(info$t2d_lipodys, all.info$t2d_lipodys),par_risk= round(info$t2d_lipodys/parsc,2))
      donor.res$t2d_liver_lipid = list(value = round(info$t2d_liver_lipid,2), perc = getPercentile(info$t2d_liver_lipid, all.info$t2d_liver_lipid),par_risk= round(info$t2d_liver_lipid/parsc,2))
      
      parsc =info$t1d_drdq + info$t1d_class1 + info$t1d_class2 + info$t1d_nonhla+4.71;
